...
- `--browser-port <port>` (alias: `--browser-debug-port`): pin the DevTools port (useful on WSL/Windows firewalls). Env aliases: `ORACLE_BROWSER_PORT` / `ORACLE_BROWSER_DEBUG_PORT`. When omitted, a random open port is chosen.
