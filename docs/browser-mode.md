...
- Config: set `browser.forceEnglishLocale=true` in `~/.oracle/config.json` to pass `--lang=en-US` and `--accept-lang=en-US,en` to Chrome when locale-sensitive selectors drift.
