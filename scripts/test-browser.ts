 ...
 * - Launches Chrome headless with a fixed DevTools port (default 45871 or env BROWSER_TEST_PORT).
...
async function main() {
  console.log(`[browser-test] launching Chrome on ${targetHost}:${port} (headless)…`);
  const chrome = await launch({
    port,
    chromeFlags: ['--headless=new', '--remote-debugging-address=0.0.0.0'],
  });

  let ok = await fetchVersion(targetHost, chrome.port);
  // Give Chrome a moment if it started slowly.
  if (!ok) {
    await sleep(500);
